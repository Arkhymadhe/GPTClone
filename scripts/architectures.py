...
class Encoder(nn.Module):
    def __init__(self, embedding_dim=128, num_embeddings=1024, num_heads=32, transform_states=True):
        super().__init__()
        self.embedding_layer = nn.Embedding(embedding_dim=embedding_dim, num_embeddings=num_embeddings)
        self.self_attention = MultiHeadAttention(num_heads=num_heads, transform_states=transform_states, hidden_dim=embedding_dim)
    ...
    def forward(self, x, mask=None):
        ...
        state_embedding = self.self_attention(embeddings, mask=mask)
        new_state_embedding = self.transform_vector(state_embedding)
...
class Decoder(nn.Module):
    def __init__(self, embedding_dim=128, num_embeddings=1024, num_heads=32, transform_states=True):
        ...
        self.self_attention = MultiHeadAttention(num_heads=num_heads, transform_states=transform_states, hidden_dim=embedding_dim)
        self.cross_attention = MultiHeadAttention(num_heads=num_heads, transform_states=transform_states, hidden_dim=embedding_dim)
    ...
    def forward(self, x, source_mask=None, target_mask=None):
        ...
        decoder_predictions = self.context_transformer(attn_embeddings_2)
...
if __name__ == "__main__":
    ...
    attn = Decoder(transform_states=False)
